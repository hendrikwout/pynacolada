...
# - specify your own order of output dimensions
# - more sensible determination of memory limitation
# - allow wild cards
# - allow multiple input files in a stream
...
# adimapply: the input dimensions of the function based on the refapplyout

# arefapply = [list([])]*len(arefs)
# adimapply = np.array([list([None]*len(refapplyout))]*len(arefs))
adimapply = np.zeros((len(arefs),len(refapplyout)),dtype='int32')

for irefapplyout,erefapplyout in enumerate(refapplyout):
    for idims,edims in enumerate(adims):
        adimapply[idims,irefapplyout] = adims[idims][np.where(arefs[idims] == erefapplyout)[0][0]]


dummydat = []
avtypeout = []
for idimapply,edimapply in enumerate(adimapply):
...
for iddout in range(len(ddout)):
    ...
    if (len(np.array(ddout[iddout]).shape) != len(adimapply[iddout])):
        raise SomeError('The amount of input ('+str(len(adimapply[iddout]))+') and output dimensions ('+str(len(ddout[iddout].shape))+') of function  is not the same')
...
adimnoiter = []
for idimsswapply,edimsswapply in enumerate(adimapply):
...
for irefapplyout,erefapplyout in enumerate(refapplyout):
    for idimsswapply,edimsswapply in enumerate(adimapply):
        adimnoiter[idimsswapply].append(int(adimapply[idimsswapply,irefapplyout]))

# for now:
#   adimnoiter = adimapply, but will be appended below
#   refnoiterout = refapplyout
# we now will try to read the data in even larger icecubes!


if (max(alennoiter) > maxicecubesize):
    print 'Warning, one of the function data input lengths "',alennoiter,'" (dimensions: ',adimapply,') exceeds the maximum icecubesize of '+str(maxicecubesize)+'.' 
else:
    ...
    while ((idim >= 0) & ((max(alennoiter)*max(adimssw[:,idim])) < maxicecubesize)):
        ...
        if (idim not in refnoiterout):
            ...
            for idimsapply,emaxdimsapply in enumerate(adimapply):
...
for j in range(lenitermax):
    ...
    for k in range(lennoapply):
        ...
        for irefs, erefs in enumerate(arefs):
            ...
            ahunk[irefs].shape = adimapply[irefs]
