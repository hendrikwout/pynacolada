...
dims_output = {
        'latitude':{ 'coordinates':ds.latitude,'chunksize':5000},
        'longitude':{ 'coordinates':ds.longitude,'chunksize':5000},
        }
...
logging.info('determining chunked shapes for each xarray')
...
logging.info('xarray chunk memory sizes: '+ str(chunks_memory_sizes))
...
for idim,dimname in iteration_over_noapply_dims:
    if (current_memory_size < maximum_memory_size_input):
        ...
        #print(current_memory_size > maximum_memory_size_input)
...
logging.info('memory input size of chunks: '+ str(current_memory_size) +'/'+str(maximum_memory_size_input) +' = '+str(current_memory_size/int(maximum_memory_size_input)*100)+'% of maximum')
if current_memory_size > maximum_memory_size_input:
...
for index_no_apply in tqdm.tqdm(chunks_no_apply):
    ...
    for dimname_apply in list(reversed(dims_apply_names)):
        if dimname_apply in number_of_chunks_apply_dims.keys():
            ...
            chunk_start.insert(0,int(dim_apply_start * dims_output[dimname_apply]['chunksize']))
            chunk_end.insert(0,int(chunk_start[0]+  dims_output[dimname_apply]['chunksize']))
            ...
            #     dim_apply_start = np.mod(idx_mod/dim_fac, number_of_chunks_apply_dims[dimname_apply] )
            #     chunk_start[0].append(int(dim_apply_start * dims_output[dimname_apply]['chunksize']))
            #     chunk_end[0].append(int(chunk_start[0]+  dims_output[dimname_apply]['chunksize']))
            #     idx_mod -= dim_apply_start*dim_fac
            #     dim_fac *= number_of_chunks_apply_dims[dimname_apply]
        else:
