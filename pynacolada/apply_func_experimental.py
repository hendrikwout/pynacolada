...
def apply_func(func,xarrays,dims_apply, method_dims_no_apply='outer',filenames_out = None, attributes = None,maximum_input_memory_chars = 10240*1024*100 ,squeeze_apply_dims = False,release=False,output_dims={},transpose_hack=True,tempfile_dir=False,initialize_array=None,copy_coordinates=False):
    ...
    logger.debug('Start loop over subsequent memory chunks')

    space_position_end = 0
    while space_position_end < space_size:
        space_position_start = space_position_end

        logger.debug('determining space indices of next memory chunks start position')

        space_dims = dims_no_apply+dims_apply
        space_shape = [dimslib[d].shape[0] in (space_dims)]
        def indices_from_position(space_position,space_shape):

            space_scan = space_position
            for ishape,shape in enumerate(space_shape):
                if ishape < len(space_shape):
                    inner_size = np.prod(space_shape[ishape+1:])
                else:
                    inner_size = 1

                space_indices.append(math.floor(shape_scan / inner_size))
                shape_scan = shape_scan - space_indices[ishape]*inner_size
            return space_indices

        space_indices_start = indices_from_position(space_position_start,space_shape)

        def get_xarray_shape_transposed(xarray, dims):
            xarray_shape_transposed = []
            if dim in xarray_in.dims:
                xarray_shape_transposed.append(xarray_in[dim].shape[0])
            else:
                xarray_shape_transposed.append(1)
            return xarray_shape_transposed


        for ishape, shape in enumerate(space_shape):
            for idim,dim in list(enumerate(space_dims)][::-1]:
                for ixarray_in,xarray_in in enumerate(xarrays):

                    array_shape_transposed = get shape_transposed(xarray_in,space_dims)

                    arrays_shape_transposed = [idim]




        logger.debug('determining array_space space position according to the current
        for ixarray_in,xarray_in in xarrays.items():
